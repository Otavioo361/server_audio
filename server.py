...
class Server(object):
        ...
        while True:
            if len(self.queue) > 0:
                ...
                for message in messages:
                    print("Mensagem rcebida: ", message)
    ...
    def receive_messagem(self, message):
        with self.lock:
    ...
    def process(self):
        while True:
            with self.request:
                if len(self.messagem) == 0:
                    time.sleep(1)
                    continue
            messagem = self.messagem[0]
            del self.messagem[0]
            messagem.process()
...
# Crie um objeto Pyro4 daemon com o endereço IP da máquina e porta 9090
